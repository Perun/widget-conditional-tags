...
- **Block-Widget-Editor.** Das Plugin setzt auf die klassische Widget-Oberfläche und die Hooks `in_widget_form`, `widget_update_callback` und `widget_display_callback`. Im Block-Editor greift die Anzeige-Logik nicht; ist der Block-Editor aktiv, weist das Plugin im Backend darauf hin und verlinkt das Plugin „Classic Widgets".
...
- Lässt sich eine gespeicherte Bedingung nicht mehr auswerten, wird das Widget ausgeblendet statt eingeblendet (fail closed) – das Verhalten bleibt im Fehlerfall vorhersagbar.
